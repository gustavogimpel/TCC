...
sigmoid = FuncaoAtivacao(lambda z:1/(1+np.power(math.e,-z)),
                      lambda a,z,y,arr_dz_w_prox:arr_dz_w_prox*a*np.subtract(1, a),
                      lambda a,z,y,arr_dz_w_prox:np.subtract(a, y)
                      )
...
leaky_relu = FuncaoAtivacao(lambda z:np.where(z>=0,1*z,0.01*z),
                      lambda a,z,y,arr_dz_w_prox:arr_dz_w_prox*np.where(z>=0,1,0.01)
                      )

tanh = FuncaoAtivacao(lambda z: (np.power(math.e,z)-np.power(math.e,-z))/((np.power(math.e,z)+np.power(math.e,-z))),
...
class Camada():
    ...
    def forward_propagation(self,mat_a_ant):
        ...
        self.qtd_un_camada_ant = mat_a_ant.shape[0]

        #Inicialize com zeros a matriz de ativacao da camada atual
        #..Novamente, verifique as dimensões da matriz
        self.mat_a = np.zeros((self.qtd_un_camada_ant,len(self.arr_unidades)))
