        ...
        for num_repeticao in range(num_repeticoes):
            ...
            for num_fold in range(val_k):
                ...
                if num_fold < val_k-1:
                    ...
                    fim_fold_to_predict = len(df_dados_rand.index)
                ...
                df_treino = pd.concat([df_dados_rand[0:ini_fold_to_predict], df_dados_rand[fim_fold_to_predict:len(df_dados_rand.index)]])
                # print(df_treino)

                #5. Crie o fold (objeto da classe Fold) para adicioná-lo no vetor
                fold = Fold(df_treino, df_to_predict, col_classe, num_folds_validacao=num_folds_validacao, num_repeticoes_validacao= num_repeticoes_validacao)
